...
function Vehicles.CreateVehicle(data, cb)
    ...
    ---print('CreateVehicle data:', data)
    
    -- Verificação inicial robusta
    if not data.vehicle or not data.vehicle.plate then
        return false, lib.print.error('Error CreateVehicle: veículo ou placa do veículo é NIL')
    end
    ...
    if not data.spawn then
        ...
        data.entity = CreateVehicleServerSetter(data.vehicle.model, data.type, data.coords.x, data.coords.y, data.coords.z, data.coords.w)
    ...
    State:set('citizenid', data.citizenid, true) --mudei 
...
function Vehicles.SetCarOwner(src, entity)
    ...
    local citizenid = citizenid(src) --- ADIOCENEI UMA FUNÇÃO PARA citizenid

    if not entity then
        data.entity = GetVehiclePedIsIn(playerped, false)
    end

    if not data.entity or not citizenid or not playerped then --- MUDEI identifier PARA citizenid
        return false, lib.print.error('SetCarOwner Missing data')
    end

    if Vehicles.Vehicles[data.entity] then
        return false, lib.print.error('SetCarOwner Este veículo já tem dono')
    end
    ...
    data.vehicleName = lib.callback.await('mVehicle:GetVehicleNameFromHash', src, props.model) -- VEICULO NOME
    data.hash = props.model -- HASH
    data.setOwner = true
    data.spawn = true
...
function Vehicles.GetControlVehicle(src, entity, identifier)
    ...
    data.vehicle = props
    data.owner = identifier
    data.citizenid = citizenid ---citizenid
    data.vehicleName = lib.callback.await('mVehicle:GetVehicleNameFromHash', src, props.model) -- VEICULO NOME
    data.hash = props.model -- HASH
...
-- Função SetVehicleOwner
function Vehicles.SetVehicleOwner(data)
    local insert = {}
    if Config.Framework == 'standalone' or Config.Framework == 'esx' or Config.Framework == 'LG' then
    ...
    elseif Config.Framework == 'qbox' then
        insert = { data.owner, data.citizenid, data.vehicleName, data.hash, data.plate, json.encode(data.vehicle), data.type, data.job, json.encode(data.coords),
            json.encode(data.metadata), data.parking }
    end
...
function Vehicles.GetVehicle(EntityId)
    ...
    self.Private = function(bucket, coords, parking)
        if bucket then
            ...
            self.private = 1
            MySQL.update('UPDATE player_vehicles SET private = 1, coords = ?, parking = ?  WHERE plate = ?',
                { json.encode(coords), parking, self.plate })
        else
            SetEntityRoutingBucket(self.entity, 0)
            self.DeleteMetadata('RoutingBucket')
            self.private = nil
            MySQL.update('UPDATE player_vehicles SET private = 0 WHERE plate = ?', { self.plate })
        end
    end
...
---GetAllVehicles   ----MUDEI TODOS PARA citizenid
function Vehicles.GetAllVehicles(src, VehicleTable, haveKeys)
    ...
    if VehicleTable then
        if citizenid then
            local veh = {}
            for k, v in pairs(Vehicles.Vehicles) do
                if v.owner == citizenid or v.license == identifier then
                    if haveKeys then
                        veh[v.entity] = v
                        return veh
                    else
        ...
        if citizenid then
            local rows = {}
            if haveKeys then
                rows = MySQL.query.await(Querys.getVehiclesbyOwnerAndhaveKeys, { citizenid, '%"' .. citizenid .. '"%' })
            else
                rows = MySQL.query.await(Querys.getVehiclesbyOwner, { citizenid, })
            end
            return rows
        end
    end
...
function Vehicles.SpawnVehicles() --- FUNCIONANDO
    ...
    for i = 1, #dbvehicles do
        ...
        if row.coords and row.stored == 0 or row.stored == nil and row.pound == nil then
            row.vehicle = json.decode(row.mods) --- ROW.VEHICLES PARA ROW.MODS MUDEI ISSO 
...
lib.callback.register('mVehicle:VehicleState', function(source, action, data)
    ...
    elseif action == 'addkey' then
        ...
        local target = citizenid(data.serverid)
        if target then
    ...
    elseif action == 'deletekey' then
        if vehicle and data.citizenid then
            vehicle.RemoveKey(data.citizenid)
        else
            MySQL.update(Querys.saveKeys, { json.encode(data.keys), data.plate })
        end
    elseif action == 'getkeys' then
        local citizenid = citizenid(source)
        return MySQL.query.await(Querys.getKeys, { citizenid })
